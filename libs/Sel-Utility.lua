...
function check_samba()
	if not (buffactive['Haste Samba'] or buffactive['Drain Samba'] or buffactive['Aspir Samba']) and windower.ffxi.get_ability_recasts()[216] and windower.ffxi.get_ability_recasts()[216] < latency and state.AutoSambaMode.value ~= 'Off' and player.tp > 400 then
		if state.AutoSambaMode.value:lower() == 'haste' then
			windower.chat.input('/ja "Haste Samba" <me>')
			tickdelay = os.clock() + 1.8
			return true
		elseif state.AutoSambaMode.value:lower() == 'aspir' then
			windower.chat.input('/ja "Aspir Samba" <me>')
			tickdelay = os.clock() + 1.8
			return true
		elseif state.AutoSambaMode.value:lower() == 'drain' then
			windower.chat.input('/ja "Drain Samba II" <me>')
			tickdelay = os.clock() + 1.8
			return true
		end
	else
	...
	if state.AutoRuneMode.value and (player.main_job == 'RUN' or (player.sub_job == 'RUN' and not buffactive['SJ Restriction'])) and not silent_check_amnesia() and not data.areas.cities:contains(world.area) then
		...
		elseif player.main_job == 'RUN' and (not buffactive[state.RuneElement.value] or buffactive[state.RuneElement.value] < 2 or not buffactive['Tenebrae']) and state.MaintainTenebrae.value then
			...
			if not buffactive['Tenebrae'] then
				windower.chat.input('/ja "Tenebrae" <me>')
			else
				windower.chat.input('/ja "'..state.RuneElement.value..'" <me>')
			end
			tickdelay = os.clock() + 1.8
...
function get_current_stratagem_count()
	...
	if player.sub_job == 'SCH' and buffactive['SJ Restriction'] then
		return
	end
...
--		local currentlocation = {X=modified:sub(5,8), Y=modified:sub(13,16)}
		local currentlocation = {X=packet.X, Y=packet.Y}
